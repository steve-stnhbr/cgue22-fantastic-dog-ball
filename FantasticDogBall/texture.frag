...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient = light.ambient.xyz * vec3(texture(diffuse, texCoords));
    vec3 diffuse = light.diffuse.xyz * diff * vec3(texture(diffuse, texCoords));
    vec3 specular = light.specular.xyz * spec * vec3(texture(specular, texCoords));
